# Common Mistakes, Improvements, and Errata aka list of corrections

If you find any mistakes in the third edition, *Apps and Services with .NET 10*, or if you have suggestions for improvements, then please [raise an issue in this repository](https://github.com/markjprice/apps-services-net10/issues) or email me at markjprice (at) gmail.com.

> **Warning!** You must be signed in to your GitHub account to enable the **New issue** button, otherwise it will mislead you by saying *Issue creation is restricted in this repository*. All signed in GitHub users are allowed to create issues in my public repositories.

[**Errata** (32 items)](errata.md): Typos, tool user interface and behavior changes, or mistakes in code that would cause a compilation error that prevents a successful build.

[**Improvements** (5 items)](improvements.md): Changes to text or code that would improve the content. These are optional.

[**Common Mistakes**](https://github.com/markjprice/markjprice/blob/main/articles/common-mistakes.md): These are some of the most common errors that a reader might encounter when trying to get code in book tasks to work, or when trying to write your own code. (Links to the `markjprice` shared articles repository.)

All errata and improvements will be included in the next edition.

# Top Contributors

Who|Errata|Improvements
---|---|---
Phil aka [zkazz](https://github.com/zkazz)|15|1
[Moisés Sirvente](https://github.com/es-moises)|3|1
[Kris](https://github.com/kprikratki)|3|1
[DrAvriLev](https://github.com/DrAvriLev)|1|2
[nthnhwrdprmn](https://github.com/nthnhwrdprmn)|1|0
[Sammy Lastre Silveira](https://github.com/Sammy-Lastre)|1|0
[Giuseppe Guerra](https://github.com/giuseppe-guerra)|1|0
Emailer|1|0
